...
class TonStation {
    ...
    async makeRequest(method, url, data = null, token = null, proxyAgent, timeout = 20) {
    ...
    async registerCheck(initData, proxyAgent, userId) {
        const url = `${this.baseURL}/userprofile/api/v1/users/${userId}/by-telegram-id`;

        try {
            const response = await this.makeRequest('GET', url, null, null, proxyAgent);
            if (response) {
                return response;
            } else {
                this.log(`Không tìm thấy thông tin tài khoản. Thử đăng kí...`, 'warning');
                const register_url = `${this.baseURL}/userprofile/api/v1/users`;
                const payload = { initData };
                const response_1 = await this.makeRequest('POST', register_url, payload, null, proxyAgent, 5);
                await new Promise(resolve => setTimeout(resolve, 1000));
                const response_2 = await this.makeRequest('GET', url, null, null, proxyAgent);
                if(response_2) {
                    if(response_2.telegramId) { //address
                        this.log(`Đăng kí tài khoản thành công !`, 'success');
                    } else {
                        this.log(`Đăng kí tài khoản thất bại.`, 'error');
                    }
                } else {
                    this.log(`Có lỗi khi đăng kí tài khoản.`, 'warning');
                }
            }
        } catch (error) {
            this.log(`Register check error: ${error.message}`, 'error');
            return null;
        }
    }

    async getPoints(token, userId, proxyAgent) {
        //https://tonstation.app/balance/api/v1/balance/0xfeb096b75337ec71309034c4d3cdd8e74612a1b2/by-address
        // response.data.balance[0].balance

        const userURL = `${this.baseURL}/userprofile/api/v1/users/${userId}/by-telegram-id`;
        try {
            const response = await this.makeRequest('GET', userURL, null, null, proxyAgent);
            const userAddress = response.address;

            const pointURL = `${this.baseURL}/balance/api/v1/balance/${userAddress}/by-address`;
            const response_1 = await this.makeRequest('GET', pointURL, null, token, proxyAgent);
            const userPoint = response_1.data.balance[0].balance;
            this.log(`Points: ${userPoint}`, 'success');
        } catch(e) {
            this.log(`Không lấy được point. ${e}`, 'warning');
        }

    }

    async dailyClaim(token, userId, proxyAgent) {
        const url = `${this.baseURL}/farming/api/v1/user-rates/login`;
        try {
            const payload = { "userId": userId.toString() };
            const response = await this.makeRequest('POST', url, payload, token, proxyAgent);
            const userLevel = response.data.level;
            this.log(`Level: ${userLevel}, profit/h: ${response.data.coinsInSecond}. Update: ${response.data.levelUpdateDate}`, 'success');
        } catch(e) {
            this.log(`Không lấy được thông tin điểm danh. ${e}`, 'warning');
        }
    }
    ...
    async main() {
        ...
        while (true) {
            for (let i = 0; i < data.length; i++) {
                ...
                // check register
                await this.registerCheck(initData, proxyAgent, userId);
                ...
                if (authResult) {
                    ...
                    await this.getPoints(accessToken, userId, proxyAgent);
                    await this.dailyClaim(accessToken, userId, proxyAgent);
